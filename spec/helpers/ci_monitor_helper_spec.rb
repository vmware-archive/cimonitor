...
describe CiMonitorHelper do
  ...
  describe 'cimonitor build box messages' do
  ...
  describe "tracker status box messages" do
    before do
    ...
    it "should show appropriate relative messages for no deadlines" do

      expected_messages = [
          ['project_published_at', "No release deadline"]
      ]
      @project.tracker_release_deadline = nil
      messages = helper.relative_tracker_status_messages_for(@project)
      messages.should == expected_messages
    end

    it "should show appropriate relative messages for deadlines in the past" do
      deadline = 3.day.ago
      @project.tracker_release_deadline = deadline

      expected_messages = [
          ['project_published_at', "Release deadline #{distance_of_time_in_words(deadline, Time.now)} ago"]
        ]
    ...
    it "should show appropriate relative messages for deadlines in the future" do
      deadline = 5.day.from_now
      @project.tracker_release_deadline = deadline
      expected_messages = [
          ['project_published_at', "Release deadline in #{distance_of_time_in_words(deadline, Time.now)}"]
        ]
